...
export abstract class BaseManager<T extends IToolkitModule> extends Component {
	...
	private cleanConfig(currentConfig: any, defaultConfig: any): any {
		...
		for (const key in defaultConfig) {
			if (key in currentConfig) {
				cleanedConfig[key] = this.cleanConfig(
					currentConfig[key],
					defaultConfig[key]
				);
			} else {
	...
	protected addCommand(command: Command): void {
		const enhancedCommand = {
			...command,
			id: command.id.includes(this.moduleId)
				? command.id
				: `${this.moduleId}.${command.id}`,
	...
	protected addMenuItem(
		...
		const pluginManager = (this.plugin as RavenHogwartsToolkitPlugin)
			.pluginManager;
	...
	protected async saveSettings(): Promise<void> {
		try {
			...
			this.logger.throwError(
				new Error(`Error saving settings for module: ${this.moduleId}`),
				error
			);
		}
	}
	...
	protected async updateConfig(
		...
		try {
			...
			this.logger.throwError(
				new Error(`Error updating config for module: ${this.moduleId}`),
				error
			);
		}
	}
	...
	protected async updateData(newData: Partial<T["data"]>): Promise<void> {
		try {
			...
			this.logger.throwError(
				new Error(`Error updating data for module: ${this.moduleId}`),
				error
			);
		}
	}
