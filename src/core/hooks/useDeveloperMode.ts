...
export const useDeveloperMode = ({
	plugin,
	onActivated,
}: UseDeveloperModeProps) => {
	const [clickCount, setClickCount] = React.useState(0);
	const [showHint, setShowHint] = React.useState(false);
	const clickTimeout = React.useRef<NodeJS.Timeout>();
	const hasBeenEnabled = Boolean(
		plugin.settings.config.developer?.enableCount > 0
	);
			...
			plugin.updateSettings({
				config: {
					...
					developer: {
						...
						enableCount: plugin.settings.config.developer?.enabled
							? plugin.settings.config.developer?.enableCount || 0
							: (plugin.settings.config.developer?.enableCount ||
									0) + 1,
					},
				},
			});
