...
interface ComboboxSuggestionProps {
	id?: string;
	name?: string;
	label?: string;
	value: string;
...
export const ComboboxSuggestion: FC<ComboboxSuggestionProps> = ({
	id,
	name,
	label,
	value,
	...
	return (
		<div className={`rht__combobox ${className || ""}`} ref={compositeRef}>
				...
				{showList && (
					<Popover.Portal>
						<Popover.Content
							...
							className="rht__combobox--content"
...
const VirtualList = forwardRef(function VirtualList(
	...
	return (
		<div
			ref={setRefs}
			className="rht__combobox--options-list"
			role="listbox"
					...
					>
						{virtualizer.getVirtualItems().map((row) => {
							...
							return (
								<div
									key={item.value}
									className={`rht__combobox--option`}
									onClick={(e) => {
										e.preventDefault();
										e.stopPropagation();
								...
								>
									<div className="rht__combobox--option-label">
										{item.icon && (
											<span className="rht__combobox--option-icon">
												{item.icon}
											</span>
										)}
										{item.label}
