...
export class TableCalculationService {
		...
		try {
			...
			if (!strategy) {
				this.logger.throwError(
					new Error(`Unsupported function: ${parsed.function}`)
				);
			}
	...
	private getColumnValues(
		...
		const columnIndices = columns.map((column) => {
			...
			if (index === -1) {
				this.logger.throwError(
					new Error(`Column not found: ${column}`)
				);
			}
			return index;
		});
	...
	private parseDate(value: string, customFormat?: string): Date | null {
		...
		if (customFormat) {
			try {
				...
				this.logger.warn(
					`Failed to parse date using custom format: ${customFormat}`
				);
			}
		}
