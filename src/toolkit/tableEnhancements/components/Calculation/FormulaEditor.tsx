...
const FormulaEditor: FC<FormulaEditorProps> = ({ table, value, onChange }) => {
	const [activeSuggestion, setActiveSuggestion] =
		useState<IFunctionHint | null>(null);
	const [hoverSuggestion, setHoverSuggestion] =
		useState<IFunctionHint | null>(null);
	...
	const insertFunction = (hint: IFunctionHint) => {
		...
		const template = (hint.defaultTemplate || hint.syntax).replace(
			"columns",
			""
		);
		onChange(template);
	...
	const insertColumn = (columnName: string) => {
		...
		const currentColumns = value
			.slice(bracketIndex + 1, closeBracketIndex)
			.split(",")
			.map((col) => col.trim())
			.filter((col) => col !== "");
	...
	const insertParameter = (param: IFunctionParameter, option: string) => {
		...
		const afterBracket = value.slice(
			closeBracketIndex + 1,
			hasCloseParen ? closeParenIndex : undefined
		);

		// 解析现有参数，同时处理单引号和双引号
		const params = new Map<string, string>();
		const paramMatches =
			afterBracket.match(/(?:,\s*)?(['"])(.*?)\1/g) || [];

		// 特殊处理 TimeSpan 函数的格式:单位参数
		if (functionName === "TimeSpan" && param.name === "unit") {
			const formatParam = activeSuggestion.parameters?.find(
				(p) => p.name === "format"
			);
			const currentFormat =
				params.get("format") || formatParam?.default || "yyyy-MM-dd";
			params.set("format", `${currentFormat}:${option}`);
		} else if (functionName === "TimeSpan" && param.name === "format") {
			// 如果是修改格式，保持现有的单位
			const unitParam = activeSuggestion.parameters?.find(
				(p) => p.name === "unit"
			);
			const currentUnit =
				params.get("unit") || unitParam?.default || "days";
			params.set(param.name, `${option}:${currentUnit}`);
		} else {
			// 其他函数的常规参数处理
			activeSuggestion.parameters?.forEach((p, index) => {
				if (paramMatches[index]) {
					const paramValue = paramMatches[index].replace(
						/^,\s*['"]|['"]$/g,
						""
					);
		...
		const newFormula = `${functionName}(${columnsPart}${
	...
	return (
		<div className="formula-editor">
			<div className="formula-columns">
				<h3>
					{t(
						"toolkit.tableEnhancements.formula_editor.table_columns"
					)}
				</h3>
					...
					{table.headers.map((column) => (
						<div
							...
							className={`column-item ${
								isColumnSelected(column.content)
									? "selected"
									: ""
							}`}
			...
			<div className="formula-input">
				<textarea
					...
					placeholder={t(
						"toolkit.tableEnhancements.formula_editor.input_placeholder"
					)}
			...
			<div className="formula-functions">
				<h3>
					{t(
						"toolkit.tableEnhancements.formula_editor.function_name"
					)}
				</h3>
						...
						{FUNCTION_HINTS.map((hint) => (
							<div
								...
								className={`function-item ${
									activeSuggestion?.function === hint.function
										? "active"
										: ""
								}`}
								onClick={() => {
					...
					<div className="function-detail-wrapper">
						{currentSuggestion && (
							<div className="function-detail">
								<div className="hint-syntax">
									{t(
										"toolkit.tableEnhancements.formula_editor.hint_syntax"
									)}
									: {currentSuggestion.syntax}
								</div>
								<div className="hint-example">
									{t(
										"toolkit.tableEnhancements.formula_editor.hint_example"
									)}
									: {currentSuggestion.example}
								</div>

								{currentSuggestion.parameters &&
									currentSuggestion.parameters.length > 0 && (
										<div className="hint-parameters">
											<div className="parameters-title">
												{t(
													"toolkit.tableEnhancements.formula_editor.hint_parameters"
												)}
												:
											</div>
											{currentSuggestion.parameters.map(
												(param, index) => (
													<div
														key={index}
													...
													>
														<div className="parameter-name">
															{param.name}
															{param.optional && (
																<span className="parameter-optional">
																	(
																	{t(
																		"toolkit.tableEnhancements.formula_editor.parameter_optional"
																	)}
																	)
																</span>
															)}
														...
														{param.options && (
															<div className="parameter-options">
																{t(
																	"toolkit.tableEnhancements.formula_editor.parameter_options"
																)}
																:{" "}
																{param.options.map(
																	(opt) => (
																		<span
																			...
																			title={`${t(
																				"toolkit.tableEnhancements.formula_editor.insert_option"
																			)}: '${opt}'`}
																		>
																			{
																				opt
