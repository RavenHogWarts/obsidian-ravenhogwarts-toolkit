...
const TableModal: FC = () => {
	...
	useEffect(() => {
		...
		// 更新表格数据
		const newGridData: ITableGridData[] = currentTable.cells.map(
			(row, rowIndex) => {
	...
	const handleCellValueChanged = useCallback(
		(params: CellValueChangedEvent) => {
			...
			// 找到对应的列索引
			const colIndex = currentTable.headers.findIndex(
				(h) => h.field === field
			);
						...
						<div className="tableEnhancements-header-right">
							<button
								className={`tableEnhancements-btn ${
									showCalculation ? "active" : ""
								}`}
								onClick={toggleCalculation}
							>
								{showCalculation
									? t(
											"toolkit.tableEnhancements.formula.hide_calculator"
									  )
									: t(
											"toolkit.tableEnhancements.formula.show_calculator"
									  )}
							</button>
							{!isEditing ? (
								<button
								...
								>
									{t(
										"toolkit.tableEnhancements.formula.edit_table"
									)}
								</button>
							) : (
								<>
									...
									>
										{t(
											"toolkit.tableEnhancements.formula.update"
										)}
									</button>
									<button
									...
									>
										{t(
											"toolkit.tableEnhancements.formula.cancel"
										)}
									</button>
								</>
							)}
						...
						<div className="tableEnhancements-grid-container">
							{currentTable ? (
								...
								<div className="tableEnhancements-no-data">
									{t(
										"toolkit.tableEnhancements.formula.no_table_data"
									)}
								</div>
							)}
						</div>
