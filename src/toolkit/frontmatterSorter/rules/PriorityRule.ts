...
export class PriorityRule extends BaseRule {
	sort(entries: IFrontMatterEntry[]): IFrontMatterEntry[] {
		// 1. 分离条目
		const { priorityEntries, regularEntries, ignoreEntries } =
			this.categorizeEntries(entries);
	...
	private categorizeEntries(entries: IFrontMatterEntry[]): {
		...
		// 对忽略条目按原始顺序排序
		result.ignoreEntries.sort(
			(a, b) =>
				(originalOrder.get(a.key) ?? 0) -
				(originalOrder.get(b.key) ?? 0)
		);

		return result;
	}

	private sortPriorityEntries(
